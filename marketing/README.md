...
| `business-card.html` / `.png` / `.pdf` | General contact card for people who haven't booked yet (same 3.5in x 2in size and layout as the review card, different job). QR goes to the website instead of the Google review link, headline is "Mobile Detailing" with an "Auto, Plane & Boat Detailing" subline instead of "Thank You," so it works as a handout/intro card rather than a post-job thank-you. |
