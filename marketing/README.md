...
| `poster-flyin.html` / `.png` / `.pdf` | Booth poster for the Orcas Fly-In, 18in x 24in, meant to be laminated and stood on an easel. Same visual system as `poster-detailing.html` (black ground, Baloo 2, coral phone accent), with one new device: a reversed white band carrying FREE / Lemonade, Scones & Cookies, which is what reads across a ramp at thirty feet. Leads with the free refreshments because that is what makes someone change direction; the business identity sits above it and the service icons below. Carries the "hands-on flying experience" credibility line, which lands harder with this audience than any other. **The PDF is a 200dpi raster at exactly 18x24in, not vector.** Headless Chromium in the build container will not embed fonts into PDFs, so a vector export would font-substitute at the print shop. `baloo2-latin.woff2` is committed alongside so the HTML renders offline; regenerate as vector from a machine where Chromium embeds fonts if a scalable file is ever needed. |
| `pricing-planes.html` / `.png` / `.pdf` | Aircraft price guide, US Letter portrait, handed out at the booth and used as a quoting reference. White ground rather than the poster's black, so it prints on any machine without burning toner; coral is reserved for the prices so the eye lands on the numbers. Built around a size-class multiplier table (single piston 1x, light twin 1.5x, cabin twin 2x, turboprop 2.75x) applied to the low end of each website range, so Knox can quote a Baron or a King Air on the spot instead of saying he'll get back to them. Carries hours per package, what each includes, how the job works on a ramp, and the Annual Care Plan discounts. Multipliers are recorded in `business-info.md` too. Same raster-PDF caveat as `poster-flyin.pdf`. |
| `booking-sheet.html` / `.png` / `.pdf` | Two-page booking form for the booth. **Page 1** is one customer per sheet: contact, what they own and where it is kept, a checkbox grid of the real service tiers split by plane/boat/car, timing, and an Office Use strip for the quote, confirmed date, deposit and follow-up. **Page 2** is a sixteen-row quick list for the busy stretch when someone only wants to leave a name. "Where she's kept" earns its place: a plane hangared at Eastsound is a Tuesday job, one that flew in from Bellingham is not, and you want to know which before naming a price. White ground, hairline rules, no fills, coral only on the section marks, since this gets printed in quantity. `.png` is page 1 only; the `.pdf` has both pages. |
| `baloo2-latin.woff2` | Baloo 2 latin variable subset (SIL Open Font License), committed so `poster-flyin.html` renders without network access. |
